...
fn human_press(to_press: &mut Vec<Vec<i32>>, a: &Vec<u8>, pos: &Vec<(usize, usize)>) {
    for c1 in a {
        for c2 in a {
            let (x1, y1) = pos[*c1 as usize];
            let (x2, y2) = pos[*c2 as usize];
            let dist = (x1 as i32 - x2 as i32).abs() + (y1 as i32 - y2 as i32).abs() + 1;
...
fn keyboard_press(
    ...
    for c in a2 {
        let mut v = vec![vec![999999; 120]; 120]; //cost to letter i using j as last direction
        let cc = *c as usize;
        v[cc][b'A' as usize] = 0;
        to_press[cc][cc] = 1;
        ...
        while !q.is_empty() {
            ...
            for (a, b, direction) in &d {
                let x2 = (x1 as i32 + *a) as usize;
                let y2 = (y1 as i32 + *b) as usize;
...
fn main() {
    ...
    let mut h_k2_press = vec![vec![999999; 120]; 120];
    human_press(&mut h_k2_press, &a2, &pos2);
    let mut k2_k2_press = vec![vec![999999; 120]; 120];
    keyboard_press(&k2, &a2, &a2, &mut k2_k2_press, &h_k2_press, &pos2);
    let mut k2_k2_press2 = vec![vec![999999; 120]; 120];
    keyboard_press(&k1, &a2, &a1, &mut k2_k2_press2, &k2_k2_press, &pos1);
