...
fn check_cpu(a: i64, v: &Vec<i64>, val: i64, pos: usize, size: usize) -> bool {
    let mut cpu = CPU3bit::new();
    cpu.a = a;
    cpu.v = v.clone();
    cpu.run();
    if cpu.output.len() == size && cpu.output[pos] == val {
        return true;
    }
    false
}

fn find(l: i64, v: &Vec<i64>, j: usize) {
    let mut l = l;
    let x = 1i64 << (3 * (j));
    for _ in 0..8 {
        if check_cpu(l, &v, v[j], j, v.len()) {
            if j == 0 {
                println!("{l}");
                process::exit(0);
            } else {
