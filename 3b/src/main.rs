...
fn main() {
    ...
    let sdo = "do()".to_string();
    let pdo = sdo.as_bytes();
    let sdont = "don't()".to_string();
    let pdont = sdont.as_bytes();
    let mut ok = true;
    for line in lines {
        let l = line.unwrap();
        let l = l.as_bytes();
        let mut i = 0;
        while i < l.len() {
            if check_pattern(i, l, pdont) {
                ok = false;
                i += pdont.len();
                continue;
            }
            if check_pattern(i, l, pdo) {
                ok = true;
                i += pdo.len();
                continue;
            }
            if ok && check_pattern(i, l, pmul) {
                i += pmul.len();
                let a;
                let b;
