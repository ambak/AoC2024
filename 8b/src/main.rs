    ...
    for (_, w) in map {
        for i in 0..w.len() {
            for j in i + 1..w.len() {
                let mut k = 0;
                loop {
                    let x1 = w[i].0 as i32 + (w[i].0 as i32 - w[j].0 as i32) * k;
                    let y1 = w[i].1 as i32 + (w[i].1 as i32 - w[j].1 as i32) * k;
                ...
                k = 0;
                loop {
                    let x2 = w[j].0 as i32 + (w[j].0 as i32 - w[i].0 as i32) * k;
                    let y2 = w[j].1 as i32 + (w[j].1 as i32 - w[i].1 as i32) * k;
